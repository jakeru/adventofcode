...
# 07:18: started
# 08:08: p1 solved
# p2 solved same day but much much later..
...
def solve2(tiles_):
    ...
    for y in range(size):
        ...
        for x in range(size):
            ...
            print("y", y, "x", x, "left", left, "above", above)
            ...
            print("y", y, "x", x, "left_hash", left_hash, "above hash", above_hash)
            attach_tile = None
            if left_hash is not None:
                ...
                if attach_tile is not None:
                    print("found tile matching left", attach_tile[0])
                    #rotate = (attach_tile[1] + 1) % 4
                    #flip = attach_tile[1] // 4
                    found = False
                    for i in range(8):
                        rotate = i // 2
                        flip = i % 2
                ...
                if attach_tile is None:
                    raise RuntimeError("No tile found to attach")
                if attach_tile is not None:
                    print("found tile matching above", attach_tile[0])
                    found = False
                    for i in range(8):
                        rotate = i // 2
                        flip = i % 2
...
#    ##    ##    ###
 #  #  #  #  #  #..."""
    ...
    for y in range(image.shape[0] - th):
        for x in range(image.shape[1] - tw):
